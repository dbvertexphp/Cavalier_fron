...
export class UserFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    // Convert string IDs to numbers to avoid SQL mismatch error
    const payload = {
      ...rawData,
      branchId: rawData.branchId ? Number(rawData.branchId) : null,
      roleId: rawData.roleId ? Number(rawData.roleId) : null,
      hodId: rawData.hodId ? Number(rawData.hodId) : null,
      teamId: rawData.teamId ? Number(rawData.teamId) : null
    };

    const request = this.isBranchForm 
    ...
    request.subscribe({
      next: () => {
        ...
        alert(this.isBranchForm ? 'Branch Saved Successfully' : 'User Saved Successfully');
        this.router.navigate([this.isBranchForm ? '/dashboard/branch' : '/dashboard/hr/employee-master']);
      },
      error: (err) => {
        this.isLoading = false;
