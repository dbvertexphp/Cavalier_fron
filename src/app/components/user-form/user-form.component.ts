...
onSubmit() {
  ...
  if (!this.validateForm()) {
    this.userForm.markAllAsTouched();
    // Validation function ke andar console.log(errors) zaroor check karein
  ...
  if (this.isBranchForm) {
    ...
    // Payload taiyar karein (Jo DB mapping aapne batayi thi)
    const finalPayload = {
      ...raw,
      EmergencyRelation: raw.emergencyRelationship // DB wali key map kar di
    };

    console.log('Sending User Data:', finalPayload);
    ...
    this.userService.registerUser(finalPayload).subscribe({
      next: (res: any) => {
        ...
        // 2. Response se UserId nikalna (Res handle logic)
        const newUserId = res.id || res.userId || (res.data && res.data.id);

        if (newUserId) {
          console.log('Saving Education & Experience for ID:', newUserId);
          
          // 3. Education aur Experience save karein
          this.saveEducation(newUserId); 
          this.saveExperience(newUserId);
          
          alert('User, Education & Experience Saved Successfully');
        } else {
          // Agar ID na mile tab bhi purana flow try karein (just in case)
          this.saveEducation(newUserId);
          this.saveExperience(newUserId);
      ...
      error: err => {
        ...
        alert('User registration fail ho gaya: ' + (err.error?.message || 'Server error'));
      }
    });
  }
}
