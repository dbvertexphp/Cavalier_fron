...
export class LeadFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
      .get(`${environment.apiUrl}/Teams`)
      .subscribe((res: any) => (this.teamList = res));
  ...
  fetchNextLeadNoFromApi(): void {
    ...
    this.http
      .get<any>(`${environment.apiUrl}/Leads/nextLeadNo`, { headers })
      .subscribe({
        next: (res) => {
  ...
  if (selectedTeamId && selectedTeamId !== '') {
    this.http.get<any>(`${environment.apiUrl}/Teams/${selectedTeamId}/details`).subscribe({
      next: (res) => {
        ...
        this.salesPersonList = res.salesTeam || [];
        this.salesCoordinator = res.salesCoordinators || [];

        const patchObj: any = {};
        if (this.salesCoordinatorsList.length > 0) patchObj.salesCoordinator = String(this.salesCoordinatorsList[0].id);
        if (this.hodsList.length > 0) patchObj.hod = String(this.hodsList[0].id);
        if (this.reportingManagersList.length > 0) patchObj.reportingManager = String(this.reportingManagersList[0].id);
        if (this.salesPersonList.length > 0) patchObj.salesPerson = String(this.salesPersonList[0].id);
...
onEditLead(id: any) {
  ...
  this.http.get<any>(`${environment.apiUrl}/Leads/${id}`).subscribe({
    next: (lead) => {
      ...
      if (lead.team) {
        this.http.get<any>(`${environment.apiUrl}/Teams/${lead.team}/details`).subscribe({
          next: (res) => {
            this.salesCoordinatorsList = res.salesCoordinators || [];
            this.hodsList = res.hods || [];
            this.reportingManagersList = res.reportingManagers || [];
            ...
            this.leadForm.patchValue({
              ...
              salesPerson: lead.salesTeam ? String(lead.salesTeam) : ''
            });
  ...
  onOrgClick(orgId: any, orgName: string) {
    if (orgId) {
      ...
      Swal.fire({
        ...
        text: "The record is missing an organization reference.",
      });
    }
  }
  ...
  getBranches() {
    this.http
      .get<any[]>(`${environment.apiUrl}/branch/list`)
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    this.http
      .post(`${environment.apiUrl}/LeadColumnSettings/save`, payload)
      .subscribe({
        error: (err) => console.error(err),
      });
  }
  ...
  sortColumn(column: string) {
    ...
    this.sortOrders[column] =
      !this.sortOrders[column] || this.sortOrders[column] === "desc"
        ? "asc"
        : "desc";
  ...
  onDeleteLead(id: any) {
    Swal.fire({
      ...
      this.http
        .delete(`${environment.apiUrl}/Leads/${id}`, { responseType: "text" })
        .subscribe({
          next: () => {
            // Update local data
  ...
  loadLeadDates(): void {
    this.allDates = [
      ...new Set(this.leads.map((lead) => this.toISODate(new Date(lead.date)))),
    ];
  }
  ...
  onSave() {
    ...
    const finalOrgId =
      orgIdValue && orgIdValue !== "" ? Number(orgIdValue) : null;

    const payload: any = {
      LeadNo: this.isEditMode ? rawValue.leadNo : this.nextLeadNo,
      Date: rawValue.date ? new Date(rawValue.date).toISOString() : null,
      ExpectedValidity: rawValue.expectedValidity
        ? new Date(rawValue.expectedValidity).toISOString()
        : null,
      Type: rawValue.type,
      ...
      SalesTeam: String(rawValue.salesPerson || ""),
    ...
    if (this.isEditMode && this.selectedLeadId) {
      this.http
        .put(`${environment.apiUrl}/Leads/${this.selectedLeadId}`, payload, {
          headers,
        })
        .subscribe({
          next: () => {
      ...
      this.http
        .post(`${environment.apiUrl}/Leads`, payload, { headers })
        .subscribe({
          next: () => {
            Swal.fire({
              icon: "success",
  ...
  filterTableByOrganization(orgName: string) {
    this.http
      .get<
        any[]
      >(`${environment.apiUrl}/Leads/search-leads`, { params: { organizationName: orgName } })
  ...
  selectTeam(team: any): void {
    const matchedTeam =
      typeof team === "object"
        ? team
        : this.teamList.find((t) => (t.teamName || t.name) === team);
    if (matchedTeam) {
  ...
  filterTableBySalesStage(stage: string) {
    this.http
      .get<
        any[]
      >(`${environment.apiUrl}/Leads/search-leads`, { params: { salesStage: stage } })
      .subscribe((res) => (this.leads = res));
  }
  ...
  filterTableBySalesProcess(process: string) {
    this.http
      .get<
        any[]
      >(`${environment.apiUrl}/Leads/search-leads`, { params: { salesProcess: process } })
      .subscribe((res) => (this.leads = res));
  }
    ...
    this.http.get<any[]>(`${environment.apiUrl}/Leads`).subscribe({
      next: (data) => {
        if (Array.isArray(data)) {
          this.leadNoList = [
            ...new Set(data.map((l) => l.leadNo).filter((val) => val)),
          ];
          this.leadOrgList = [
            ...new Set(
              data.map((l) => l.organizationName).filter((val) => val),
            ),
          ];
          this.allSalesProcesses = [
            ...new Set(data.map((l) => l.salesProcess).filter((val) => val)),
          ];
          this.cdr.detectChanges();
        }
      },
  ...
  onLeadSearch() {
    ...
    if (searchInput && searchInput !== "") {
      ...
      filtersToSend = {
        ...
        OrganisationId: this.OrganisationId
          ? Number(this.OrganisationId)
          : null,
        Type:
          this.leadSearchFilters.type === "Any"
            ? ""
            : this.leadSearchFilters.type,
        LeadOwner:
          this.leadSearchFilters.leadOwner === "Any"
            ? ""
            : this.leadSearchFilters.leadOwner,
        ...
        Status:
          this.leadSearchFilters.status === "Any" ||
          !this.leadSearchFilters.status
            ? ""
            : this.leadSearchFilters.status.toString(),
    ...
    this.http
      .post<
        any[]
      >(`${environment.apiUrl}/Leads/Search`, filtersToSend, { headers })
      .subscribe({
        next: (response) => {
          this.leads = response || [];
          this.updatePagination();
  ...
  resetLeadFilters() {
    ...
    this.http
      .post<any[]>(
        `${environment.apiUrl}/Leads/Search`,
        {
          LeadNo: "",
          OrganizationName: "",
          Type: "",
  ...
  downloadLeadsPDF() {
    ...
    doc.rect(10, 25, 277, 10, "F");
    doc.setFontSize(7);
    doc.setTextColor(255, 255, 255);
    this.selectedColumns.forEach((col, i) =>
      doc.text(col.toUpperCase(), 10 + i * colWidth + 2, 32),
    );
    ...
    this.leads.forEach((l, rIndex) => {
      ...
      this.selectedColumns.forEach((col, cIndex) => {
        ...
        doc.text(
          val.toString().substring(0, 17),
          10 + cIndex * colWidth + 2,
          y,
        );
      });
      y += 8;
  ...
  printLeads() {
    ...
    this.selectedColumns.forEach(
      (c) =>
        (header += `<th style="padding:10px; border:1px solid #ddd;">${c}</th>`),
    );
    header += "</tr>";
    let rows = "";
    ...
    w?.document.write(
      `<html><body><h2>LEAD RECORDS SUMMARY</h2><table style="width:100%; border-collapse:collapse;"><thead>${header}</thead><tbody>${rows}</tbody></table></body></html>`,
    );
    w?.document.close();
    setTimeout(() => {
      w?.print();
  ...
  downloadLeadsExcel() {
    ...
    const data = this.leads.map((lead) => {
      let r: any = {};
      this.selectedColumns.forEach(
        (c) => (r[c] = lead[this.columnFieldMap[c]] ?? "-"),
      );
      return r;
    });
  ...
  oniconLeadSearch() {
    ...
    this.iconSearchSub = this.http
      .get<any[]>(`${environment.apiUrl}/Leads`)
      .subscribe((res) => {
        const u = [...new Set(res.map((i) => i.organizationName))].filter(
          (n) => n,
        );
  ...
  oniconHODSearch() {
    ...
    if (this.OnlyleadOwner && this.OnlyleadOwner.length > 0) {
      this.allHODList = [
        ...new Set(this.OnlyleadOwner.map((u) => u.name)),
      ...
      this.userServices.getHodList().subscribe({
        next: (res: any[]) => {
          this.OnlyleadOwner = res;
          this.allHODList = [...new Set(res.map((u) => u.name))].filter(
            (n) => n,
          );
          this.allHODListFiltered = [...this.allHODList];
          this.cdr.detectChanges();
  ...
  onHODSearchType(event: Event): void {
    ...
    this.filteredHODSuggestions =
      v.length >= 3
        ? this.allHODList.filter((h) => h.toLowerCase().includes(v))
        : [];
  }
  ...
  filterHODModalList() {
    ...
    this.allHODListFiltered = q
      ? this.allHODList.filter((h) => h.toLowerCase().includes(q))
      : [...this.allHODList];
  }
  ...
  onLeadOwnerIconClick() {
    ...
    if (this.OnlyleadOwner && this.OnlyleadOwner.length > 0) {
      ...
      this.userServices.getHodList().subscribe({
        next: (res: any[]) => {
          this.OnlyleadOwner = res;
          this.leadOwnerList = [...new Set(res.map((u) => u.name))].filter(
            (n) => n,
          );
          this.allOwnersFiltered = [...this.leadOwnerList];
          this.cdr.detectChanges();
  ...
  onLeadOwnerSearch(event: Event): void {
    ...
    this.filteredLeadOwners =
      v.length >= 3
        ? this.leadOwnerList.filter((o) => o.toLowerCase().includes(v))
        : [];
  }
  ...
  filterOwnerModalList() {
    ...
    this.allOwnersFiltered = q
      ? this.leadOwnerList.filter((o) => o.toLowerCase().includes(q))
      : [...this.leadOwnerList];
  }
  ...
  onLeadNoIconClick() {
    this.isLNModalOpen = true;
    this.modalLNSearchText = "";
    this.http
      .get<
        any[]
      >(`${environment.apiUrl}/Leads`, { headers: { Authorization: `Bearer ${localStorage.getItem("cavalier_token")}` } })
      .subscribe((res) => {
        this.allLNList = [
          ...new Set(res.map((i) => i.inquiryNo || i.leadNo || i)),
        ].filter((v) => v);
  ...
  onTeamIconClick() {
    ...
    this.http
      .get<
        any[]
      >(`${environment.apiUrl}/Teams`, { headers: { Authorization: `Bearer ${localStorage.getItem("cavalier_token")}` } })
      .subscribe((res) => {
        this.allTeamsList = [
          ...new Set(res.map((t) => t.teamName || t.name || t)),
        ];
  ...
  onManagerIconClick() {
    ...
    if (this.OnlyleadOwner && this.OnlyleadOwner.length > 0) {
      ...
      this.userServices.getHodList().subscribe({
        next: (res: any[]) => {
          this.OnlyleadOwner = res;
          this.allManagersList = [...new Set(res.map((u) => u.name))].filter(
            (n) => n,
          );
          this.rmIconListFiltered = [...this.allManagersList];
          this.cdr.detectChanges();
  ...
  filterManagers(event?: any) {
    ...
    this.rmIconListFiltered = this.allManagersList.filter((m) =>
      m.toString().toLowerCase().includes(q),
    );
    this.cdr.detectChanges();
  }
  ...
  loadOrganizationList() {
    ...
    this.http
      .get<
        any[]
      >(`${environment.apiUrl}/Organization/List`, { headers: { Authorization: `Bearer ${localStorage.getItem("cavalier_token")}` } })
      .subscribe({
        next: (res) => {
          this.orgList = res;
  ...
  loadLeadOwners(): void {
    this.userServices
      .getHodList()
      .subscribe({ next: (res: any) => (this.hodList = res) });
  }
  ...
  confirmSelection() {
    ...
    if (selected.length === total || selected.length === 0) {
      ...
      this.leadSearchFilters.branch = selected
        .map((b) => b.branchName)
        .join(", ");
  ...
  toggleLeadStatus(lead: any) {
    ...
    this.http
      .patch(`${environment.apiUrl}/Leads/UpdateStatus/${lead.id}`, newStatus, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("cavalier_token")}`,
  ...
  getsales(): void {
    this.userServices
      .getHodList()
      .subscribe({ next: (data: any[]) => (this.OnlyleadOwner = data) });
  }
}
