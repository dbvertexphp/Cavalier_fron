...
// --- Fix for Error TS2339 (columnFieldMap) ---
// Isse HTML ko pata chalega ki kis column header ke liye konsa data dikhana hai
columnFieldMap: any = {
  'Lead No': 'leadNo',
...
// --- Fix for Error TS2339 (sortColumn) ---
// Ye function sorting ka logic handle karega
sortColumn(col: string) {
  const field = this.columnFieldMap[col];
  if (!field) return;

  // Toggle order: asc -> desc -> asc
  const currentOrder = this.sortOrders[col] || 'desc';
  const newOrder = currentOrder === 'asc' ? 'desc' : 'asc';
  
  // Clear other column orders and set this one
  this.sortOrders = {}; 
  this.sortOrders[col] = newOrder;
