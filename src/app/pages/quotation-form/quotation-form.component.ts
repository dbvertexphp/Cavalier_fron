...
export class QuotationFormComponent implements OnInit {
  ...
  dimRows: any[] = [{ box: null, l: null, w: null, h: null, unit: "CMS" }];
  ...
  // ✅ INDEPENDENT DUAL PIPELINE CALENDAR EXECUTION LIFECYCLE PIPELINES
  ...
  isCellBeforeStart(cellDateString: string | null): boolean {
    // Sirf tab active hoga jab start date select ho chuki ho aur end date abhi select nahi hui
    if (
      !cellDateString ||
    ...
    return new Date(cellDateString) < new Date(this.customStartSelectedDate);
  }
  ...
  markAgentAsSelected(agentName: string, attempt: number = 0) {
    ...
    if (this.agentDetail && this.agentDetail.length > 0) {
      ...
      if (match) {
        ...
        console.log("✅ Agent auto-marked in review:", match.agentName);
        this.cdr.detectChanges();
        return;
      }
    }

    // Agar agentDetail abhi load nahi hui, thodi der baad dobara try karo (max 5 baar)
    if (attempt < 5) {
  ...
  fetchAgentByLobId(lobId: string | number, countryName: string) {
    if (!lobId) {
      console.warn(
        "⚠️ Operation blocked: Line of Business parameter target reference layout missing.",
      );
      return;
    }
    ...
    console.log(
      "📡 Dispatching Combined Agent Query Payload directly matching structural bounds:",
      url,
    );
    ...
    this.http.get<any[]>(url).subscribe({
      next: (res) => {
        ...
        // 🔥 FIX: Jo Agent "Carrier / Agent" dropdown se already select kiya hua hai
        // usko yahan fresh list aane ke baad dobara tick/mark karo
        if (this.quotation.carrierAgent) {
          this.markAgentAsSelected(this.quotation.carrierAgent);
        }

        this.cdr.detectChanges();
        console.log(
          "✅ Agents/Branches synced for review layout matrix context:",
          this.agentDetail,
        );
      },
      error: (err) => {
        console.error(
          "❌ Agent dynamic fetch protocol execution failure trace:",
          err,
        );
        this.agentDetail = [];
        this.cdr.detectChanges();
  ...
  onAgentSelect(event: any, agent: any) {
    ...
    console.log(
      "Current Selection matrix tracking trace snapshot:",
  ...
  sendBulkEmails(qtnId: number) {
    if (!this.selectedEmails || this.selectedEmails.size === 0) {
      console.warn("⚠️ No target agents marked, bypassing execution stream.");
      return;
    }

    // 🔥 FIX: Pricing ki asli ID bhejo, Quotation ki nahi
    const linkedPricingId = this.quotation.pricingId;

    if (!linkedPricingId) {
      console.warn(
        "⚠️ Is quotation ke saath koi Pricing record linked nahi hai, email skip.",
      );
      Swal.fire(
        "Warning",
        "This quotation is not linked to any Pricing record, so email cannot be sent.",
        "warning",
      );
      return;
    }
    ...
    const payload = {
      ...
      inquiryId: linkedPricingId, // 👈 Ab Pricing.Id match hoga
    ...
    this.http
      .post(`${environment.apiUrl}/Pricing/SendBulkEmail`, payload, { headers })
      .subscribe({
        next: () => {
          Swal.fire({
            icon: "success",
        ...
        error: (err) => {
          console.error(
            "❌ Bulk Mail API pipeline error profile trace rejection:",
            err,
          );
          Swal.fire({
            icon: "error",
            ...
            text:
              err.error?.message ||
              "Something went wrong while sending bulk email array dataset matrix references.",
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    this.http
      .post(`${environment.apiUrl}/QuotationColumnSetting/save`, payload)
  ...
  loadColumnSettings() {
    this.http
      .get<any>(`${environment.apiUrl}/QuotationColumnSetting`)
    ...
    this.http.get<any>(url).subscribe({
      next: (fullData) => {
        ...
        if (fullData.transportMode) {
          ...
          const matchedMode = this.transportModes?.find(
            (m) => m.name.toLowerCase() === modeFromApi.toLowerCase(),
          );
          this.quotation.transportMode = matchedMode
            ? matchedMode.name
            : modeFromApi;
        } else {
        ...
        if (fullData.transportType) {
          ...
          this.quotation.transportMode?.toUpperCase() === "AIR" ||
          this.quotation.transportMode?.toUpperCase() === "SEA"
        ) {
        ...
        // --- Remaining Fields ---
        this.quotation.businessDimensions =
          fullData.businessDimensions ||
          fullData.businessDim ||
          fullData.commodityName ||
          "";
        ...
        this.quotation.commodity = fullData.commodity
          ? String(fullData.commodity)
          : "";
        ...
        const statusDate = fullData.cargoStatusDate
        ...
        this.quotation.version = fullData.version || 0;
  ...
  onSearchInput() {
    ...
    if (this.quotation.organization && this.quotation.organization.length > 0) {
      ...
      this.filteredOrganizations = this.organizations.filter((org) =>
        org.orgName
          .toLowerCase()
          .includes(this.quotation.organization.toLowerCase()),
      );
    } else {
...
onCountryBlur() {
  ...
  saveQuotation() {
    ...
    const validRevenueRows = this.revenueRows.filter(
      (r) => r.chargeName && r.chargeName.trim() !== "",
    );
    const validCostRows = this.costRows.filter(
      (c) => c.chargeName && c.chargeName.trim() !== "",
    );

    if (validRevenueRows.length === 0 && validCostRows.length === 0) {
      Swal.fire(
        "Incomplete Data",
        "Valid Revenue or Cost items required!",
        "warning",
      );
      return;
    }
    ...
    const payload = {
      ...
      version:
        this.quotation.version !== null &&
        this.quotation.version !== undefined &&
        this.quotation.version !== ""
          ? Number(this.quotation.version)
          : null,
      ...
      pricingId: this.quotation.pricingId
        ? Number(this.quotation.pricingId)
        : null,
      organisationId: this.quotation.organisationId
        ? Number(this.quotation.organisationId)
        : null,
      organisationName:
        this.quotation.organisationName || this.quotation.organization,
      ...
      transitDest: this.quotation.transitDays
        ? `${this.quotation.transitDest} (${this.quotation.transitDays} Days)`
        : this.quotation.transitDest,
      ...
      dimensionsData:
        this.dimRows && this.dimRows.length > 0
          ? JSON.stringify(this.dimRows)
          : "",
      ...
      multiCarriers: this.multiCarrierRows
        ? this.multiCarrierRows.map((m) => ({
            ...
            cutoff: m.cutoff,
            schedule:
              m.schedule && m.schedule !== "0001-01-01" && m.schedule !== ""
                ? new Date(m.schedule).toISOString()
                : null,
            currency: m.currency,
        ...
        : [],
      portOfLoadingId:
        this.quotation.portOfLoadingId !== null &&
        this.quotation.portOfLoadingId !== undefined &&
        this.quotation.portOfLoadingId !== ""
          ? Number(this.quotation.portOfLoadingId)
          : null,
      portOfDischargeId:
        this.quotation.portOfDischargeId !== null &&
        this.quotation.portOfDischargeId !== undefined &&
    ...
    const request =
      this.quotation.id > 0
        ? this.http.put(
            `${this.apiEndpoint}/update/${this.quotation.id}`,
            payload,
            httpOptions,
          )
    ...
    request.subscribe({
      next: (res: any) => {
        ...
        this.http
          .post(
            `${this.apiEndpoint}/SaveQuotationRevenue`,
            revenuePayload,
            httpOptions,
          )
          .subscribe({
            next: () => {
              ...
              this.http
                .post(
                  `${this.apiEndpoint}/SaveQuotationCost`,
                  costPayload,
                  httpOptions,
                )
                .subscribe({
                  next: () => {
                    ...
                    this.http
                      .post(
                        `${this.apiEndpoint}/SavePnLSummary`,
                        pnlPayload,
                        httpOptions,
                      )
                      .subscribe({
                        next: () => {
                          ...
                          if (
                            ...
                            console.log(
                              "🚀 Quotation entries compiled successfully. Transitioning control stream to bulk mailing matrix architecture...",
                            );
  ...
  onTeamChange(teamId: any) {
    console.log(
      "👥 Team Selected inside Quotation Framework. Team ID:",
      teamId,
    );
    ...
    if (
      ...
      this.getsalesTeamList = []; // Reset Sales Team list
      return;
    }
  ...
  editQuotation(q: any) {
    ...
    this.quotation = {
      ...
      dimUnit: q.dimensionsData
        ? JSON.parse(q.dimensionsData)[0]?.unit || "CMS"
        : "CMS",
      // chargeableWeight: q.chrgWeight || 0,
      // grossWeightKg: q.grossWeight || q.grossWeightKg || 0,
      ...
      cbm:
        q.cbmWeight ||
        (Number(q.volumeWeight)
          ? parseFloat((q.volumeWeight / 167).toFixed(3))
          : 0),
      ...
      portOfLoadingId:
        q.portOfLoadingId !== null &&
        q.portOfLoadingId !== undefined &&
        q.portOfLoadingId !== "" &&
        !isNaN(Number(q.portOfLoadingId))
          ? Number(q.portOfLoadingId)
          : null,
      portOfDischargeId:
        q.portOfDischargeId !== null &&
        q.portOfDischargeId !== undefined &&
        q.portOfDischargeId !== "" &&
        !isNaN(Number(q.portOfDischargeId))
          ? Number(q.portOfDischargeId)
          : null,
      salesTeam: q.salesTeam || q.SalesTeam || "",
      carrierCode: q.carrierCode || q.CarrierCode || "",
    ...
    if (this.quotation.portOfLoadingId) {
      const pol = this.portOfLoadingList?.find(
        (p) => p.id === this.quotation.portOfLoadingId,
      );
      if (pol) {
    ...
    if (this.quotation.portOfDischargeId) {
      const pod = this.portOfDischargeList?.find(
        (p) => p.id === this.quotation.portOfDischargeId,
      );
      if (pod) {
    ...
    if (q.connectingPortIds) {
      const idsArray =
        typeof q.connectingPortIds === "string"
          ? q.connectingPortIds
              .split(",")
              .map((x: any) => x.trim())
              .filter((x: any) => x !== "")
          : Array.isArray(q.connectingPortIds)
            ? q.connectingPortIds
            : [q.connectingPortIds];
      ...
      this.selectedConnectingPorts = idsArray.map((id: any) => {
        ...
        const masterPort = this.filteredConnectingPorts?.find(
          (p) => p.id.toString() === trimmedId,
        );
      ...
      console.log(
        "✅ Populated selectedConnectingPorts:",
        this.selectedConnectingPorts,
      );
    }
...
if (q.cargoValue) {
  ...
  this.quotation.cargoValue = null;
    ...
    if (q.transitDest && q.transitDest.includes("(")) {
      ...
      this.quotation.transitDays =
        daysMatch && daysMatch[1] ? daysMatch[1].trim() : "";
    ...
    if (q.cargoStatus === "Ready" || q.cargoStatus === "Ready By") {
      this.quotation.cargoReadyDate = q.validFrom
    ...
    this.revenueRows = q.revenueData
      ? typeof q.revenueData === "string"
        ? JSON.parse(q.revenueData)
        : q.revenueData
      : [];
    this.costRows = q.costData
      ? typeof q.costData === "string"
        ? JSON.parse(q.costData)
        : q.costData
      : [];
    ...
    this.dimRows = dimData
      ? typeof dimData === "string"
        ? JSON.parse(dimData)
        : dimData
      : [{ box: null, l: null, w: null, h: null, unit: "CMS" }];
  ...
  addRevenueRow() {
    const currentLOB = this.quotation.lineOfBusiness;
  ...
  calculateAll() {
    ...
    allCharges.forEach((charge) => {
      ...
      const finalLOB =
        revRow?.lob || costRow?.lob || this.quotation.lineOfBusiness || "N/A";

      // 5. Profit calculation
      const profit = revForCharge - costForCharge;
      const profitPercent =
        revForCharge !== 0 ? (profit / revForCharge) * 100 : 0;
  ...
  prepareQuotationPayload() {
    const payload = {
      ...
      profitPercentage:
        this.totalRevFinal !== 0
          ? (this.totalProfitFinal / this.totalRevFinal) * 100
          : 0,
    };
    return payload;
  }
  ...
  openDimModal() {
    ...
    // 2. Agar API se data nahi aaya, tab hi quotation wali backup values dekho
      ...
      this.quotation.allDimensions &&
      this.quotation.allDimensions.length > 0
    ) {
  ...
  loadSearchSuggestions() {
    ...
    this.http
      .get<any[]>(`${environment.apiUrl}/Quotations`, { headers })
      .subscribe({
        next: (data) => {
          if (Array.isArray(data)) {
            // --- Aapka purana logic START ---
            this.quotationNoList = [
              ...new Set(data.map((q) => q.quotationNo).filter((val) => val)),
            ];
            this.quotedByList = [
              ...new Set(
                data
            ...
            this.organizationList = [
              ...new Set(data.map((q) => q.organization).filter((val) => val)),
            ];
  ...
  onLOBType() {
    const query = this.searchFilters.lineOfBusiness
      ? this.searchFilters.lineOfBusiness.trim().toLowerCase()
      : "";
  ...
  onQuotationNoType() {
    const query = this.searchFilters.quotationNo
      ? this.searchFilters.quotationNo.trim().toLowerCase()
      : "";
  ...
  onOrgType() {
    const query = this.searchFilters.organization
      ? this.searchFilters.organization.trim().toLowerCase()
      : "";
  ...
  // 🔥 Typing logic: 3 characters ke baad suggestions filter honge
  onQuotedByType() {
    const query = this.searchFilters.quotedBy
      ? this.searchFilters.quotedBy.trim().toLowerCase()
      : "";
  ...
  onSearch() {
    ...
    filtersToSend.serviceType =
      selectedLOB && selectedLOB !== "Any" ? selectedLOB : "";

    // Clean strings params fields mapping
    if (filtersToSend.lineOfBusiness === "Any")
      filtersToSend.lineOfBusiness = "";
    ...
    forkJoin({
      ...
      error: (err) => {
        ...
        alert(
          "Search execution encountered an error. Please verify input parameters.",
        );
      },
    });
  }
  ...
  fetchInitialQuotations() {
    this.http
      .get<any[]>(`${this.apiEndpoint}/GetAll`) // Ya jo bhi aapka main GET route ho
      .subscribe({
        next: (res) => {
          this.quotations = res || [];
          this.cdr.detectChanges();
  ...
  resetFilters() {
    ...
    this.http
      .post<any[]>(`${this.apiEndpoint}/Search`, resetPayload)
      .subscribe({
        next: (response) => {
  ...
  printRecords() {
    ...
    this.quotations.forEach((q, idx) => {
      ...
      activeCols.forEach((col) => {
        const fieldKey = this.columnFieldMap[col] || col;
        let val =
          q[fieldKey] !== null && q[fieldKey] !== undefined ? q[fieldKey] : "-";
  ...
  onHeaderLOBChange() {
    ...
    if (selectedName) {
      ...
      if (newLOBId) {
        this.fetchAgentByLobId(newLOBId, this.quotation.country || "");
      }
    }

    // Baki logic...
    if (this.revenueRows)
      this.revenueRows.forEach((row) => (row.lob = selectedName));
  ...
  onPageSizeChange() {
    ...
    const maxSafetyPages =
      Math.ceil(this.quotations.length / this.pageSize) || 1;
      ...
      this.lobSub = this.http
        ...
        .subscribe({
          next: (res) => {
            ...
            this.allLOBs = [...new Set(cleanedList)].filter(
              (val) => val !== null && val !== "" && val !== "[object Object]",
            );
  ...
  toggleQuotedByPopup() {
    if (this.showQuotedByPopup) {
      ...
      this.quotedBySub = forkJoin({
        quotations: this.http.get<any[]>(`${environment.apiUrl}/Quotations`, {
          headers,
        }),
        hodList: this.userServices.getHodList(),
      }).subscribe({
        next: (res) => {
          ...
          const hodMap = new Map(
            hodList.map((h: any) => [String(h.id), h.name]),
          );
          ...
          this.allSalesCoors = [...new Set(cleanedCoors)].filter(
            (val) => val !== null && val !== "" && val !== "[object Object]",
          );

          console.log(
            "📂 Final SalesCoors List with Names:",
            this.allSalesCoors,
          );
  ...
  loadPricingList() {
    ...
    this.http
      .get(`${environment.apiUrl}/Pricing/GetPricingList`, { headers })
      .subscribe({
        next: (res: any) => {
  ...
  onPricingSearchInput() {
    ...
    if (searchText.length > 0) {
      this.showPricingDropdown = true;
      this.filteredPricings = this.pricingList.filter(
        (p) =>
          p.pricingNo.toLowerCase().includes(searchText) ||
          (p.organisationName &&
            p.organisationName.toLowerCase().includes(searchText)),
      );
    } else {
      this.showPricingDropdown = false;
    }
    ...
    this.http.get<any>(url).subscribe({
      next: (p) => {
        ...
        this.quotation.customerName =
          p.OrganisationName || prc.customerName || "";
        ...
        if (p.cargoStatusDate)
          this.quotation.cargoReadyDate = p.cargoStatusDate.split("T")[0];
        this.quotation.salesCoordinator = Number(
          p.SalesCoordinatorId || p.salesCoordinator || 0,
        );
        this.quotation.salesTeam = p.salesTeam || p.SalesTeam || "";
        this.quotation.teamId = p.teamId ? Number(p.teamId) : null;
        // 🔥 Carrier Name (Code + Name)
        this.quotation.carrierCode = p.carrierCode || p.CarrierCode || "";
        this.quotation.awbIssuedBy = p.carrierName || p.CarrierName || "";

        // 🔥 Carrier / Agent
        this.quotation.carrierAgent = p.agent || p.Agent || "";
        console.log(
          "🔍 Mapped TeamID:",
          this.quotation.teamId,
          "from API:",
          p.teamId,
        );
        ...
        setTimeout(() => {
          if (p.pricingDoneBy || p.PricingDoneBy) {
            const targetP = (p.pricingDoneBy || p.PricingDoneBy)
              .toString()
              .toLowerCase();
            const foundP = this.getpricingByList.find(
              (item) =>
                item.id.toString() === targetP ||
                item.name.toLowerCase() === targetP,
            );

            // Agar dropdown list me name mil jata hai toh use set karo, nahi toh original string fallback rakho
            this.quotation.pricingBy = foundP
              ? foundP.name
              : p.pricingDoneBy || p.PricingDoneBy;
          }
          this.cdr.detectChanges();
        }, 200); // Mapped response timeout closure safely managed
        // 2. Multi-Carrier Breakdown Mapping (Exact Database Keys Match)
        if (
          p.multiCarrierBreakdowns &&
          Array.isArray(p.multiCarrierBreakdowns)
        ) {
        ...
        if (p.costBreakdowns && Array.isArray(p.costBreakdowns)) {
          ...
          // 🔥 REVENUE table ko COST table se auto-fill karo (a-to-z copy)
          this.revenueRows = this.costRows.map((c: any) => ({
            lob: c.lob || "",
            chargeName: c.chargeName || "",
            chargeType: c.chargeType || "",
            currency: c.currency || "",
          ...
          console.log("✅ Cost Rows:", this.costRows);
          console.log("✅ Revenue Rows Auto-Filled:", this.revenueRows);
        }
        ...
        console.log("Tables Updated:", {
          cost: this.costRows,
          multi: this.multiCarrierRows,
        });
        // --- Business Dimensions ---
        this.quotation.businessDimensions =
          p.businessDimensions || p.BusinessDimensions || "";
        ...
        const matchedMode = this.transportModes?.find(
          (m) =>
            m.id == apiMode || m.name.toLowerCase() == apiMode.toLowerCase(),
        );
        ...
        this.quotation.placeOfDelivery =
          p.placeOfDelivery || p.PlaceOfDelivery || p.deliveryPlace || "";
        // --- POL Mapping ---
        // 1. ID Map karo
        ...
        this.quotation.portOfLoadingCode =
          p.portOfLoadingCode || p.PortOfLoadingCode || "";
        ...
        this.quotation.portOfDischargeCode =
          p.portOfDischargeCode || p.PortOfDischargeCode || "";
        ...
        this.quotation.finalDestinationCode =
          p.CodeOfFinalDest || p.codeOfFinalDest || "";
        console.log(
          "Final Destination Code Set To:",
          this.quotation.finalDestinationCode,
        );
        this.quotation.podFinalDest =
          p.podFinalDest || p.FinalDestination || "";
        ...
        this.quotation.chargeableWeight = Number(
          p.chargeableWeight || p.ChargeableWeight || 0,
        );
        this.quotation.chrgWeight = this.quotation.chargeableWeight;
        this.quotation.chargeableWeightUnit =
          p.chargeableWeightUnit || p.ChargeableWeightUnit || "KGS";
        ...
        const matchedUnit = this.packageUnits?.find(
          (u) => u.name.toLowerCase() === apiUnit.toLowerCase(),
        );
        this.quotation.pkgUnit = matchedUnit ? matchedUnit.name : apiUnit;
        // --- 6. Weights, Volume & CBM ---
        /// --- 6. Weights & CBM ---
        this.quotation.chargeableWeightKg = Number(
          p.volumeWeight || p.ChargeableWeight || 0,
        );
        this.quotation.volumeWeightUnit =
          p.chargeWeightUnit || p.VolumeWeightUnit || "KGS";
        // --- Description Mapping ---
        ...
        const matchedInco = this.incoTerms?.find(
          (i) =>
        ...
        const matchedMove = this.movementTypes?.find(
          (m) => m.name.toLowerCase() === apiMove.toString().toLowerCase(),
        );
        ...
        this.quotation.cargoValue =
          p.cargoValue || p.CargoValue || p.value || 0;
        ...
        this.quotation.commodity = Number(
          p.commodityId || p.CommodityId || p.commodity || 0,
        );

        // --- 4. Route, Country & Location ---
        this.quotation.originPOL = p.originName || p.origin || "";
        this.quotation.podFinalDest =
          p.finalDestination || p.FinalDestination || "";
        this.quotation.placeOfDelivery =
          p.placeOfDelivery || p.PlaceOfDelivery || "";
        ...
        if (cpIds) {
          ...
          this.selectedConnectingPorts =
            this.filteredConnectingPorts?.filter((port) =>
              idsArray.includes(Number(port.id)),
            ) || [];
        }

        // --- 6. Weights & CBM ---
        this.quotation.grossWeightKg = Number(
          p.grossWeightKg || p.GrossWeightKg || 0,
        );
        this.quotation.volumeWeight = Number(
          p.volumeWeight || p.VolumeWeight || 0,
        );
        this.quotation.cbm = parseFloat(
          Number(p.cbm || p.TotalCbm || 0).toFixed(3),
        );

        // --- 7. Business & Sales Info ---
        this.quotation.lineOfBusiness =
          p.lineOfBusinessId || p.LineOfBusinessId || "";
        ...
        this.quotation.isDirect =
          p.isDirect !== undefined ? p.isDirect : p.IsDirect || false;
        this.quotation.isIndirect =
          p.isIndirect !== undefined ? p.isIndirect : p.IsIndirect || false;
  ...
  loadPortOfLoadings() {
    ...
    this.http.get(`${environment.apiUrl}/PortSetup`, { headers }).subscribe({
      next: (data: any) => {
        ...
        console.log(
          "⚓ POL Data Loaded directly from Environment:",
  ...
  loadPortOfDischarges() {
    ...
    this.http.get(`${environment.apiUrl}/PortSetup`, { headers }).subscribe({
      next: (data: any) => {
        ...
        console.log(
          "⚓ POD Data Loaded directly from Environment:",
          this.portOfDischargeList,
        );
      },
  ...
  onPolChange() {
    const selectedPol = this.portOfLoadingList.find(
      (p) => p.id == this.quotation.portOfLoadingId,
    );
    this.quotation.portOfLoadingName = selectedPol
      ? selectedPol.portName || selectedPol.name
      : "";
  ...
  onPodChange() {
    const selectedPod = this.portOfDischargeList.find(
      (p) => p.id == this.quotation.portOfDischargeId,
    );
    this.quotation.podSearchTerm = selectedPod
      ? selectedPod.portName || selectedPod.name
      : "";
    this.quotation.portOfDischargeCode = selectedPod
  ...
  updatePreview(): void {
    if (this.quotation) {
      this.quotation.chrgWeight = this.quotation.chargeableWeight || 0;
  ...
  calculateTotalRevenue(): number {
    return this.revenueRows.reduce(
  ...
  isHazard(): boolean {
    ...
    const selected = this.commodityTypes?.find(
      (c) => c.id == this.quotation.commodity,
    );
  ...
  checkHazardStatus() {
    ...
    const selected = this.commodityTypes.find(
      (c) => String(c.id) === String(this.quotation.commodity),
    );
  ...
  initializeAllUnits() {
    if (this.uomList.length > 0) {
      ...
      if (!this.quotation.grossWeightUnit)
        this.quotation.grossWeightUnit = targetUnit;
      if (!this.quotation.netWeightUnit)
        this.quotation.netWeightUnit = targetUnit;
      if (!this.quotation.chargeableWeightUnit)
        this.quotation.chargeableWeightUnit = targetUnit;
      if (!this.quotation.volumeWeightUnit)
        this.quotation.volumeWeightUnit = targetUnit;
  ...
  onFinalDestinationSearch() {
    const searchTerm = (this.quotation.podFinalDest || "")
      .toString()
      .trim()
      .toLowerCase();
  ...
  onCodeInput() {
    const code = (this.quotation.finalDestinationCode || "")
      .toString()
      .trim()
      .toLowerCase();

    // Auto-fill logic
    const match = this.portsOfLoading.find(
      (p) => p.portCode?.toString().toLowerCase() === code,
    );
    if (match) {
  ...
  onFinalKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "ArrowDown") {
      event.preventDefault();
      if (this.highlightedIndex < this.filteredFinalDestinations.length - 1)
      ...
      if (this.highlightedIndex > -1) {
        this.selectFinalDestination(
          this.filteredFinalDestinations[this.highlightedIndex],
        );
      }
    }
  }
...
onCountrySearch() {
  ...
  openPricingModal() {
    ...
    if (!this.pricingList || this.pricingList.length === 0) {
      ...
      console.log(
        "Pricing list already loaded, skipping API call.",
        this.pricingList,
      );
  ...
  onPolKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "ArrowDown") {
      event.preventDefault();
      this.highlightedPolIndex =
        (this.highlightedPolIndex + 1) % this.filteredPolList.length;
    } else if (event.key === "ArrowUp") {
      event.preventDefault();
      this.highlightedPolIndex =
        (this.highlightedPolIndex - 1 + this.filteredPolList.length) %
        this.filteredPolList.length;
    } else if (event.key === "Enter") {
      event.preventDefault();
  ...
  onPodSearch() {
    ...
    const searchTerm = (
      this.quotation.podSearchTerm ||
      this.quotation.portOfDischargeCode ||
      ""
    ).toLowerCase();

    this.filteredPodList = this.portOfDischargeList.filter(
      (p) =>
  ...
  onPodKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "ArrowDown") {
      event.preventDefault();
      this.highlightedPodIndex =
        (this.highlightedPodIndex + 1) % this.filteredPodList.length;
    } else if (event.key === "ArrowUp") {
      event.preventDefault();
      this.highlightedPodIndex =
        (this.highlightedPodIndex - 1 + this.filteredPodList.length) %
        this.filteredPodList.length;
    } else if (event.key === "Enter") {
      event.preventDefault();
  ...
  // ================= CARRIER NAME (Airline) — Code + Name Search =================
  airlineList: any[] = [];
  ...
  fetchAirlines() {
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (err) => console.error("Error fetching airlines:", err),
      });
  }

  // Code se search
  onCarrierCodeInput() {
    const code = (this.quotation.carrierCode || "").toLowerCase();
    this.filteredCarrierList = this.airlineList.filter((a) =>
      (a.airlineCode || a.code || "").toLowerCase().includes(code),
    );
  ...
  // Name se search
  onCarrierNameSearch() {
    const name = (this.quotation.awbIssuedBy || "").toLowerCase();
    this.filteredCarrierList = this.airlineList.filter((a) =>
      (a.airlineName || a.name || "").toLowerCase().includes(name),
    );
    this.showCarrierDropdown = this.filteredCarrierList.length > 0;
    this.highlightedCarrierIndex = -1;
  }

  // Select karne par dono fields (Code + Name) fill honge
  selectCarrier(airline: any) {
    this.quotation.carrierCode = airline.airlineCode || airline.code || "";
    this.quotation.awbIssuedBy = airline.airlineName || airline.name || "";
  ...
  onCarrierKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "ArrowDown") {
      ...
      this.highlightedCarrierIndex =
        (this.highlightedCarrierIndex + 1) % this.filteredCarrierList.length;
    } else if (event.key === "ArrowUp") {
      event.preventDefault();
      this.highlightedCarrierIndex =
        (this.highlightedCarrierIndex - 1 + this.filteredCarrierList.length) %
        this.filteredCarrierList.length;
    } else if (event.key === "Enter") {
      event.preventDefault();
      if (this.highlightedCarrierIndex > -1) {
        this.selectCarrier(
          this.filteredCarrierList[this.highlightedCarrierIndex],
        );
      } else {
        this.selectCarrier(this.filteredCarrierList[0]);
      }
    }
  }
